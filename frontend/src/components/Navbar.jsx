...
const serviceGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Industrial design registration", path: "/services/industrial-design" },
  ...
  },
  {
    ...
    items: [
      { name: "IP portfolio management", path: "/services/ip-portfolio-management" },
  ...
  return (
    <header
      className={`sticky top-0 z-50 border-b transition-colors duration-300 ${
        scrolled ? "bg-[var(--brand-surface)]/85 backdrop-blur-md border-[var(--brand-border)]" : "bg-[var(--brand-surface)] border-transparent"
      }`}
    >
      <div className="max-w-8xl mx-auto px-8 lg:px-16 xl:px-20">
        ...
        >
          ...
          >
            ...
            />
            <div>
              <h2 className="text-[26px] font-serif leading-none tracking-tight text-[var(--brand-text)]">
          ...
          <nav className="hidden lg:flex items-center gap-9">
            {homeLink && <NavLink item={homeLink} active={isActive(homeLink.path)} />}
            {aboutLink && <NavLink item={aboutLink} active={isActive(aboutLink.path)} />}
                    ...
                    {serviceGroups.map((group, i) => (
                      <button
                        ...
                        className={`group/tab w-full flex items-center justify-between gap-2 px-5 py-3 text-left text-sm font-medium transition-colors ${
                          activeGroup === i ? "bg-[var(--brand-surface)]/10 text-[var(--brand-gold)]" : "text-white/70 hover:text-white"
                        }`}
                      >
                        ...
                        <ChevronRight
                          ...
                          className={`transition-all duration-200 ${
                            activeGroup === i ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-1 group-hover/tab:opacity-40"
                          }`}
                        />
                      </button>
                    ))}
                  ...
                  <div className="flex-1 p-6 min-h-[280px]">
                    {serviceGroups.map((group, i) => (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-400 mb-4">{group.blurb}</p>
      ...
      >
        ...
        >
          <div className="flex items-center justify-between px-6 h-20 border-b border-[var(--brand-border)]">
            <span className="text-lg font-serif text-[var(--brand-text)]">Menu</span>
            <button
              onClick={() => setIsOpen(false)}
          ...
          <div className="divide-y divide-[var(--brand-border)]">
            {homeLink && <MobileLink item={homeLink} active={isActive(homeLink.path)} onClick={() => setIsOpen(false)} />}
            {aboutLink && <MobileLink item={aboutLink} active={isActive(aboutLink.path)} onClick={() => setIsOpen(false)} />}
            ...
            {restLinks.map((item) => (
              <MobileLink key={item.path} item={item} active={isActive(item.path)} onClick={() => setIsOpen(false)} />
            ))}
          </div>
  ...
  return (
    <Link
      ...
      className={`group relative py-2 text-[15px] font-medium tracking-wide transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-gold)] focus-visible:ring-offset-2 rounded ${
        active ? "text-[var(--brand-gold)]" : "text-[var(--brand-text)] hover:text-[var(--brand-gold)]"
      }`}
    >
      {item.name}
      <span
  ...
  return (
    <Link
      ...
      className={`relative block px-6 py-4 text-[16px] font-medium transition-colors ${
        active ? "text-[var(--brand-gold)] bg-[var(--brand-gold)]/5" : "text-[var(--brand-text)] hover:bg-[var(--brand-bg)] active:bg-[var(--brand-surface)]"
      }`}
    >
      {active && <span className="absolute left-0 top-0 bottom-0 w-[3px] bg-[var(--brand-gold)]" />}
      {item.name}
    </Link>
  );
}